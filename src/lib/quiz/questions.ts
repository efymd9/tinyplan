// ── Quiz Screen Types & Data (v4 — Smart Plan Builder) ─────────────────────
...
// ── Helpers for conditions & display ────────────────────────────────────────

function answerIncludes(answers: Record<string, string | string[]>, screenId: string, value: string): boolean {
  const val = answers[screenId];
  if (!val) return false;
  return Array.isArray(val) ? val.includes(value) : val === value;
}

function answerIs(answers: Record<string, string | string[]>, screenId: string, value: string): boolean {
  const val = answers[screenId];
  if (!val) return false;
  return Array.isArray(val) ? val[0] === value : val === value;
}

function getFirst(answers: Record<string, string | string[]>, screenId: string): string {
  const val = answers[screenId];
  if (!val) return '';
...
const CHILD_STYLE_TO_PROFILE: Record<string, { name: string; desc: string }> = {
...
export const quizScreens: QuizScreen[] = [
...

  {
    ...
    dynamicText: (answers) => {
      ...
      const painTexts = painArr.map((p) => PAIN_DISPLAY[p] ?? p).join(' and ');
      const momentText = MOMENT_DISPLAY[moment] ?? moment;
...

  {
    ...
    dynamicText: (answers) => {
      ...
      const info = CHILD_STYLE_TO_PROFILE[primary];
      if (info) {
...

  {
    ...
    dynamicText: (answers) => {
      ...
      const ageStr = AGE_DISPLAY[age] ?? 'your child';
      const painStr = PAIN_DISPLAY[pain] ?? 'your goals';
      const momentStr = MOMENT_DISPLAY[moment] ?? 'your day';
      const timeStr = TIME_DISPLAY[time] ?? 'flexible';
...

  {
    ...
    dynamicLoadingTexts: (answers) => {
      ...
      const profileInfo = CHILD_STYLE_TO_PROFILE[primaryStyle];

      const ageStr = AGE_DISPLAY[age] ?? 'your child';
      const painStr = PAIN_DISPLAY[pain] ?? 'your goals';
      const momentStr = MOMENT_DISPLAY[moment] ?? 'your routine';
...
// ── Helpers for computing visible screens ──────────────────────────────────

export function getVisibleScreens(answers: Record<string, string | string[]>): QuizScreen[] {
  return quizScreens.filter((s) => !s.condition || s.condition(answers));
}
